...
void ConsultarProdutoDescricao() {
    ...
    while (fread(&produto,sizeof(produto),1,fpprodutos) == 1) {
        if(strcmp(pesquisaProduto,produto.descricao)!= NULL){//se a palavra pesquisada for diferente de NULL, encontrou
            achou = 1;
            printf("%s, %i, Estoque %i, Estoque mínimo %i, R$ %.2f", produto.descricao, produto.codigo, produto.estoque, produto.estoqueMinimo, produto.preco);
        }//FimIf.
    }//FimWhile
    fclose(fpprodutos);
...
void listarCadastro() {
    ...
    while (fread(&produto,sizeof(produto),1,fpprodutos) == 1) {
        ...
        printf("\nEstoque Mínimo: %i ", produto.estoqueMinimo);
        printf("\nPreço R$ %.2f ", produto.preco);
        printf("\n********************************************");
        printf("\n");
    }
