...
void vendasPorCliente(){
    ...
    //abre os arquivos de venda, cliente e produto
        fpvenda = fopen(CADVENDA, "rb");fpproduto = fopen(CADPRODUTO, "rb");
        ...
        if (achou){
            ...
            while ( (fread(&vendas, sizeof(vendas), 1, fpvenda) == 1) ) {//lê cada registro de cliente
                if (vendas.cod_cliente == pesquisa_cliente) {
        ...
        fclose(fpproduto);fclose(fpvenda);//fecha o arquivo de vendas
